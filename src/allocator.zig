// code taken from https://github.com/ryupold/examples-raylib.zig/blob/main/src/allocator.zig and modified
...
const e_allocator_vtable = Allocator.VTable{
    ...
    .remap = EmscriptenAllocator.remap,
};
...
const EmscriptenAllocator = struct {
    ...
    fn alignedAlloc(len: usize, log2_align: u8) ?[*]u8 {
        const alignment = @as(usize, 1) << @as(u5, @intCast(log2_align));
    ...
    fn alloc(
        ...
        _ = return_address;
        assert(len > 0);
        return alignedAlloc(len, @intFromEnum(log2_align));
    }
    ...
    fn remap(
        ...
        _ = new_len;
        _ = ret_addr;
        // Emscripten doesn't support realloc efficiently, so we don't support remap
        return null;
    }
};
